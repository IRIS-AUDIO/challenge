...
def merge_complex_specs(background, 
    ...
    ## filter ##
    if filterHz != 0:
        complex_spec *= tf.concat([tf.zeros_like(complex_spec[:int(filterHz // (16000/512))]), tf.ones_like(complex_spec[int(filterHz // (16000/512)):])], 0)
...
def make_pipeline(backgrounds, # a list of backgrounds noises
    ...
    dataset = dataset.map(partial(merge_complex_specs,config=config,n_classes=n_classes,**kwargs))
    return dataset
