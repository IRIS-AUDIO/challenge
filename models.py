...
def multi_head_attention(d_model, n_heads, output_dim=None):
    ...
    def mha(q, k, v):
        ...
        return tf.keras.layers.Dense(output_dim)(attn)
    return mha
...
def encoder(n_layers, d_model, n_heads, output_dim=16, dff=None, rate=0.1, softmax=True):
    if dff is None:
        dff = d_model * 4
    ...
    def _encoder(x):
        ...
        x = multi_head_attention(d_model, n_heads, output_dim)(embedding, x, x)
