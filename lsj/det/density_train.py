...
args.add_argument('--voices', type=str,
                  default='/codes/generate_wavs/voice_normed_complex_v2_2.pickle')
args.add_argument('--labels', type=str,
                  default='/codes/generate_wavs/voice_labels_mfc_v2_1.npy')
...
def augment(specs, labels, time_axis=-2, freq_axis=-3):
    ...
    specs = random_shift(specs, axis=freq_axis, width=2)
...
if __name__ == "__main__":
    ...
    if True:
        ...
        out = tf.keras.layers.Permute((2, 3, 1))(x) # [batch, freq, time, chan] -> [batch, time, chan, freq]
        out = out * tf.keras.layers.Dense(config.n_mels, activation='tanh')(out)
        out = tf.keras.layers.Permute((3, 1, 2))(out) # [batch, time, chan, freq] -> [batch, freq, time, chan]
        ...
        callbacks = [
            CSVLogger(NAME.replace('.h5', '.log'),
                      append=True),
            LearningRateScheduler(custom_scheduler(4096,
                                                   TOTAL_EPOCH/12,
                                                   config.lr_div)),
